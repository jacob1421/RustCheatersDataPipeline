...
def transform_achievement_fact(**kwargs):
    ...
    # Format datetime
    df['unlocktime'] = pd.to_datetime(df['unlocktime'], unit="s", errors="coerce", utc=True).dt.strftime(
        "%Y-%m-%d %H:%M:%S")
...
def transform_game_playtime_fact(**kwargs):
    ...
    # #Select columns
    df = df[["steam_id", "game_id", "date", "playtime_windows_mins", "playtime_mac_mins", "playtime_linux_mins", "playtime_two_weeks_mins"]]
...
def transform_bans_fact(**kwargs):
    ...
    #Select columns
    df = df[["steam_id", "last_ban_date", "num_vac_bans", "num_game_bans", "community_banned", "economy_ban", "vac_banned"]]
...
def transform_friends_fact(**kwargs):
    ...
    # Format datetime
    df['friend_since'] = pd.to_datetime(df['friend_since'], unit="s", errors="coerce", utc=True).dt.strftime(
        "%Y-%m-%d %H:%M:%S")
...
def transform_badges_fact(**kwargs):
    ...
    # Parse JSON into DataFrame
    df = pd.json_normalize(data["responses"], record_path=["response", "badges"], meta=["queried_steam_id", ["badges", "player_level"]], errors="ignore")
    ...
    #Select columns
    df = df[["steam_id", "badge_id", "app_id", "community_item_id", "xp", "level", "completion_time", "scarcity", "steam_level"]]
